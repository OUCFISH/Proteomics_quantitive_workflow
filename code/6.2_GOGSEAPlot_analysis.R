...
save_plot_fun <- function(GSEAplot_all, outdir, contrast, pathway_name, Anno_df, showgene, gene_label) {
  ...
  save_plot(combined_plot, outdir = outdir, saveName = paste0(contrast, pathway_id, "_", save_group), width = 12, height = 8, draw = FALSE)
...
main <- function(conf) {
  ...
  AddFont(font_path)
  showtext_auto()
  outdir <- "result/6_GSEA/6.1_GO_GSEA"
...
source(conf)
indir <- "result/6_GSEA/6.1_GO_GSEA/"

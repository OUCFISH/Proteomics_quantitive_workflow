  ...
  if (ES_value) {
    if (max_or_min == "max") {
      ...
      p.res <- p.res +
        ...
        geom_text(data = gsdata[which(gsdata$runningScore == max(gsdata$runningScore)), ], aes(y = runningScore, x = x + nrow(gsdata)/20), 
      ...
      p.res <- p.res +
        ...
        geom_text(data = gsdata[which(gsdata$runningScore == min(gsdata$runningScore)), ], aes(y = runningScore, x = x - nrow(gsdata)/20), 
                  label = ES_label, color = ES_font_color, size = ES_font_size, family = ES_font_family) +
        theme(
          legend.position = c(.8, .8), legend.title = element_blank(),
          legend.background = element_rect(fill = "transparent")
...
save_plot_fun <- function(GSEAplot_all, outdir, contrast, pathway_name, Anno_df, showgene, gene_label) {
  library(stringr)
  pathway_id <- str_extract(pathway_name, "\\(([^\\)]+)\\)$")
  ...
  save_plot(combined_plot, outdir = outdir, saveName = paste0(contrast, pathway_id, "_", save_group), width = 10, height = 8, draw = FALSE)
  ...
  write.xlsx(GSEA_pathway_df, paste0(outdir, "/", contrast, pathway_id, "_", save_group, ".xlsx"))
...
main <- function(conf) {
  ...
  for (contrast in contrast_df$contrast) { ## 对每个比较组的文件进行循环处理
    print(contrast)
    ...
    if (file.exists(GSEA_result_path)) {

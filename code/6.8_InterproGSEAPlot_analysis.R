    ...
    if (max_or_min == "max") {
      ...
      p.res <- p.res +
        ...
        geom_text(data = gsdata[which(gsdata$runningScore == min(gsdata$runningScore)), ], 
                  aes(y = runningScore, x = x - nrow(gsdata)/20), label = ES_label, color = ES_font_color, 
                  size = ES_font_size, family = ES_font_family) +
        theme(
          legend.position = "c(.8, .8)", legend.title = element_blank(),
          legend.background = element_rect(fill = "transparent")
        ) + scale_x_continuous(expand = c(0, 0))
...
main <- function(conf) {
  ...
  for (contrast in contrast_df$contrast) { ## 对每个比较组的文件进行循环处理
    ...
    if(type == "Domain") subname <- "Domain_GSEA_result.RData"
    ## 加载文件
...
source(conf)
indir <- "result/6_GSEA/6.4_Domain_GSEA/"

...
suppressWarnings(suppressMessages(library(Peptides)))
suppressWarnings(suppressMessages(library(SeqKnn)))
# 定义文件路径和参数
...
dif_p <- pvalue
##读入蛋白数据
protein.data.ori <- read.delim(protein_file,header = T,check.names = F)
peptide.data.ori <- read.delim(peptide_file,header = T,check.names = F)
# 修改列名以提取样本名
process_proteins_colnames <- function(data, group_num, sample_num) {
  colnames(data)[6 : ncol(data)] <- sub(".*/([^/_]*)_.*", "\\1", colnames(data)[6 : ncol(data)])
  # colnames(data)[6 : ncol(data)] <- str_replace(str_extract(colnames(data)[6 : ncol(data)], "([A-Z0-9]+)_"),"_","")
  return(data)
}
process_peptides_colnames <- function(data, group_num, sample_num) {
  colnames(data)[11 : ncol(data)] <- sub(".*/([^/_]*)_.*", "\\1", colnames(data)[11 : ncol(data)])
  # colnames(data)[11 : ncol(data)] <- str_replace(str_extract(colnames(data)[11 : ncol(data)], "([A-Z0-9]+)_"),"_","")
  return(data)
}
protein.data <- process_proteins_colnames(protein.data.ori, group_num, sample_num)
peptide.data <- process_peptides_colnames(peptide.data.ori, group_num, sample_num)
write.xlsx(protein.data,file = paste0(output1,"ProteinGroup.xlsx"))
write.xlsx(peptide.data,file = paste0(output1,"Peptide.xlsx"))
# 处理蛋白质ID和基因列
process_ids_genes <- function(data) {
  fasta.data <- readAAStringSet(fasta_file)
  data$Protein.Accessions <- sapply(strsplit(data$Protein.Group, split = ";"), function(x) x[1])
  fasta.name <- sapply(strsplit(names(fasta.data),"\\|"), function(x) x[2])
  match.sequence <- fasta.data[fasta.name %in% data$Protein.Accessions]
  match.name <- names(match.sequence)
  GeneName <- sapply(
    1:length(match.name), function(k) {
      GeneName <- array()
      if(length(grep("GN=", match.name[k]))==0){GeneName[k] <- NA} #2代表蛋白Description所在列数
      else{d <- regexpr("GN=", match.name[k]) #查找在data_ori_matrix[i,2]中第一次出现“GN=”的位置
      e <- substring(match.name[k],d+3) #若找到“GN=”，在第一次出现“GN=”的位置将其及其前面的字符切割
      f <- regexpr(" ",e) #查找在e中第一次出现“ ”的位置
      GeneName[k] <- substring(e, 0, f-1)} #在第一次出现“ ”的位置将其及其前面的字符切割
      GeneName[k] <- gsub("/.*$","",GeneName[k])
    }
  )
  match.df <- data.frame(protein = sapply(strsplit(match.name,"\\|"), function(x) x[2]),
                         GeneName = GeneName,
                         Description = sapply(strsplit(match.name,"\\|"), function(x) x[3]))
  
  data$Gene.Name <- match.df$GeneName[base::match(data$Protein.Accessions,match.df$protein)]
  data$Protein.Description <- match.df$Description[base::match(data$Protein.Accessions,match.df$protein)]
  data <- data[,-c(2,3,4,5)]
  data <- data %>% dplyr::select(Protein.Group,Protein.Accessions,Gene.Name,Protein.Description,everything())
  return(data)
}
protein.data <- process_ids_genes(protein.data)
...
## 最小值的二分之一
intensity_matrix <- as.matrix(data_filtered[, -c(1:nonsamplecol)]) # 去除前几列非样本列
...
if (any(grepl("QC", colnames(data_filtered)))) {
  ...
  pca.data <- protein_data_impute_addqc[, c(2, 5:ncol(protein_data_impute_addqc))]
} else {
  # 如果不包含"QC"，则仅合并填补后的数据
  protein_data_impute_merge <- do.call(cbind, protein_data_impute_group)
  protein_data_impute <- data.frame(data_filtered[, c(1:nonsamplecol)], protein_data_impute_merge)
  pca.data <- protein_data_impute[, c(2, 5:ncol(protein_data_impute))]
}
...
## 计算等电点和覆盖度
all_sequences <- readAAStringSet(fasta_file)
# 提取与UniProt ID匹配的序列信息
all_sequences_name <- sapply(strsplit(names(all_sequences),"\\|"), function(x) x[2])
matched_sequences <- all_sequences[all_sequences_name %in% data_filtered$Protein.Accessions]
protein.fasta <- data.frame(
  UniProt_ID = sapply(strsplit(names(matched_sequences),"\\|"), function(x) x[2]),
  Sequence = as.character(matched_sequences)
)
protein.fasta$pI <- sapply(protein.fasta$Sequence, function(seq) {
  Peptides::pI(seq)
})
## 计算蛋白的覆盖度coverage
peptide.data$Protein.Group <- sapply(strsplit(peptide.data$Protein.Group,";"),FUN = function(x) x[1])
peptide.data.select <- peptide.data[c("Protein.Group","Stripped.Sequence")]
matched_peptide_sequences <- all_sequences[all_sequences_name %in% peptide.data.select$Protein.Group]
peptide.fasta <- data.frame(
  UniProt_ID = sapply(strsplit(names(matched_peptide_sequences),"\\|"), function(x) x[2]),
  Sequence = as.character(matched_peptide_sequences)
)
protein.coverage <- left_join(x = peptide.data.select,y = peptide.fasta,by = c("Protein.Group" = "UniProt_ID"))
protein.coverage <- protein.coverage %>%
  filter(!is.na(Sequence) & Sequence != "", !is.na(Stripped.Sequence) & Stripped.Sequence != "")
# 定义计算覆盖度的函数
calculate_coverage <- function(protein_sequence, detected_peptides) {
  protein_length <- nchar(protein_sequence)
  if (protein_length == 0) {
    return(NA)
  }
  coverage_map <- rep(0, protein_length)
  for (peptide in detected_peptides) {
    if (nchar(peptide) == 0) {
      next
    }
    start <- unlist(gregexpr(peptide, protein_sequence, fixed = TRUE))[[1]]
    if (start != -1) {
      indices <- start:(start + nchar(peptide) - 1)
      coverage_map[indices] <- 1
    }
  }
  coverage <- sum(coverage_map) / protein_length * 100
  return(coverage)
}
# 处理数据并计算覆盖度
coverage_results <- protein.coverage %>%
  group_by(Protein.Group) %>%
  summarise(
    Coverage = calculate_coverage(
      unique(Sequence), 
      unique(Stripped.Sequence)
    )
  )
# 匹配Uniprot注释并保存结果
annotate_proteins <- function(data, uniprot_file, kinase_file, organism, output_file) {
  uniprot_db <- read.xlsx(uniprot_file)
  uniprot_db_filter <- uniprot_db %>% dplyr::select(Entry, Length, Mass,`Gene.Ontology.(GO)`, KEGG)
  data_annotated <- left_join(data, uniprot_db_filter, by = c("Protein.Accessions" = "Entry"))
  new_col_order <- c(
    "Protein.Group", "Protein.Accessions", "Gene.Name", "Protein.Description", 
    "Length", "Mass", 
    setdiff(colnames(data_annotated), c("Protein.Group", "Protein.Accessions", "Gene.Name", "Protein.Description", "Length", "Mass"))
  )
  data_annotated <- data_annotated[, new_col_order]
  colnames(data_annotated)[which(colnames(data_annotated) == "Mass")] <- "Molecular Weight[KDa]"
  data_annotated$`Molecular Weight[KDa]` <- as.numeric(data_annotated$`Molecular Weight[KDa]`) / 1000
  ...
  matched_indices <- match(data_annotated$Protein.Accessions,protein.fasta$UniProt_ID)
  data_annotated$Isoelectric.Point <- protein.fasta$pI[matched_indices]
  data_annotated <- data_annotated %>%
    dplyr::select(1:6, Isoelectric.Point, everything())
  matched_indices_coverage <- match(data_annotated$Protein.Accessions,coverage_results$Protein.Group)
  data_annotated$Coverage <- coverage_results$Coverage[matched_indices_coverage]
  data_annotated <- data_annotated %>%
    dplyr::select(1:7, Coverage, everything())
...
annotate_proteins_diff <- function(data, uniprot_file, kinase_file, organism, output_file) {
  uniprot_db <- read.xlsx(uniprot_file)
  uniprot_db_filter <- uniprot_db %>% dplyr::select(Entry,`Gene.Ontology.(GO)`, KEGG)
  data_annotated <- left_join(data, uniprot_db_filter, by = c("Protein.Accessions" = "Entry"))
  new_col_order <- c(
    "Protein.Group", "Protein.Accessions", "Gene.Name", "Protein.Description", 
    setdiff(colnames(data_annotated), c("Protein.Group", "Protein.Accessions", "Gene.Name", "Protein.Description"))
  )
  data_annotated <- data_annotated[, new_col_order]
  # kegg:K-number,pathway_id
  kegg_db <- read.csv(kegg_file)
...
for (j in 1:length(compare.name)) {
  ...
  ## diff prot
  diff.group.col <- c(which(colnames(protein_data_impute) %in% c(mean1,mean2,subgroup1,subgroup2)),
                      grep(pattern = compare.name[j],x = colnames(protein_data_impute)))
  protein_data_impute_diff <- protein_data_impute[,c(1:nonsamplecol,diff.group.col)]

...
get_KEGG_anno_df <- function(pathway2gene) {
  pathwayInfo <- read.csv(pathway2gene)
  ...
  pathwayInfo$KEGGID<-paste(organism_code,pathwayInfo$GeneID,sep=":")
  term2gene_pathway <- as.data.frame(cbind(paste0(pathwayInfo$pathway.description, "(", pathwayInfo$pathway.id, ")"), pathwayInfo[, "KEGGID"]))
  
  colnames(term2gene_pathway) <- c("pathway", "symbol")
  term2gene_pathway <- term2gene_pathway[apply(term2gene_pathway, 1, function(row) all(row != "")), ]
...
get_genelist <- function(diffExp_df, Anno_df, type) {
  diffExp_df <-select(diffExp_df,contains(c("Protein.Accessions","KEGG","Log2fc")))%>%
    separate_rows(KEGG,sep = ";")%>%filter(KEGG!="")
  colnames(diffExp_df)<-c("Protein","Genes","Logfc")
  ...
  names(genelist) <- genelist_df$Genes
  
  Anno_df <- as.data.frame(Anno_df)
  genelist <- genelist[names(genelist) %in% unique(Anno_df[,2])]
...
GSEA_anno <- function(suboutdir, contrast, genelist, Anno_GO_df, group_info, minGSGOSize, maxGSGOSize, pvalueCutoff, pAdjustMethod, type) {
  ...
  GSEA_result <- GSEA(genelist,
                      TERM2GENE = as.data.frame(Anno_GO_df[, c("pathway", "symbol")]),
  ...
  if (nrow(as.data.table(GSEA_result)) == 0) {
    error_empty_plot(err_info = "The enrichment analysis results are empty.", suboutdir)
  }else{
...
error_empty_plot <- function(err_info = "The enrichment analysis results are empty.", suboutdir) {
  error_plot <- ggplot() +
    theme_void() +
    geom_text(aes(0, 0, label = err_info), size = 5) +
    xlab(NULL)
  
  ggsave(paste0(suboutdir, "/error_info.png"), error_plot,dpi = 300,bg = "white")
}
...
main <- function(conf) {
  ...
  # source(all_plot_R)
  outdir <- "result/6_GSEA/6.2_KEGG_GSEA"
  if (!dir.exists(outdir)) {
    dir.create(outdir,recursive = T)
  } ## 创建输出文件夹
  type <- "KEGG"
  contrast_df <- get_contrast(contrast.file)
  pathway2gene <- paste0(kegg_file_path,organism_category,"/",organism_code,"_KO_pathway_description.csv")
  ...
  if (type == "GO" && file.exists(GO_RData)) {
    ...
    Anno_sub_df <- get_KEGG_anno_df(pathway2gene)
  }
  ## 对每个比较组的文件进行循环处理
  for (contrast in contrast_df$contrast) {
    ...
    if (file.exists(diffExp_path)) {
      ...
      GSEA_result <- GSEA_anno(suboutdir, contrast, genelist, Anno_GO_df=Anno_sub_df, group_info, minGSGOSize, maxGSGOSize, pvalueCutoff, pAdjustMethod, type) ## GSEA 富集分析
      GSEA_df <- GSEA_result@result

...
addAnnotation <- function(hpList, markGeneDf, DEGDt, exp, group, mapdt, color.ls, pp){
  ...
  if (pp$anno_box) {
    panel_fun <- function(index, nm) {
      ...
      # trend plot grobs
      if (pp$anno_box_line) {
        lapply(1:nrow(tmpmat), function(gRow){
          grid::grid.lines(x = scales::rescale(1:ncol(tmpmat),to = c(0,1)),
                           y = scales::rescale(tmpmat[gRow, ],
                                               to = c(0, 1),
                                               from = c(rg[1] - 0.5,rg[2] + 0.5)),
                           gp = grid::gpar(lwd = pp$anno_box_line_width, col = pp$anno_box_line_color)
          )
        })
      }
      
      # choose method for mline
      if (mlineMethod == "mean") {
        mdia <- colMeans(tmpmat)
      }else if (mlineMethod == "median") {
    ...
    mlineMethod <- pp$anno_box_mline_method
    ...
    boxlineHa <- ComplexHeatmap::anno_link(align_to = subgroup,
                                      which = "row",
                                      panel_fun = panel_fun,
    ...
    if (pp$anno_box_side == "right") {
      rightAnnoList$boxHaline <- boxlineHa
      rightAnnoList$annotation_name_gp <- gpar(fontsize = pp$hpb_legend_title_fontsize,
                                               fontfamily =  pp$hpb_legend_title_fontfamily)
      rightAnnoList$annotation_name_side <- pp$anno_geneSlice_name_side
      rightAnnoList$annotation_name_rot <- pp$hpb_colnames_rot
    }else{
      leftAnnoList$boxHaline <- boxlineHa
      leftAnnoList$annotation_name_gp <- gpar(fontsize = pp$hpb_legend_title_fontsize,
                                              fontfamily =  pp$hpb_legend_title_fontfamily)
      leftAnnoList$annotation_name_side <- pp$anno_geneSlice_name_side
      leftAnnoList$annotation_name_rot <- pp$hpb_colnames_rot
...
addLegend <- function(hp, scaleMethod, DEGDt, exp, group, mapdt, color.continuous.ls, color.ls, pp){
  ...
  regDf <- DEGDt[match(rownames(exp), Protein.Accessions)][,`:=`(Protein.Accessions = NULL,Gene.Name = NULL)] %>% setDF()
  colorNames <- unique(unlist(regDf))
  if (pp$anno_regulation_anno) {
...
main <- function(conf) {
  if (conf != "") {
    # source("11.2_heatmap_trendplot.conf")
    source(conf)
  }
  # 创建outdir
  ...
  save_plot(p = hp, outdir = outdir, saveName = "heatmapTrendplot",
            width = hp_size[1] + 1,

...
def create_sample_info():
    ...
    with open('./table/sample_info.txt', 'w', encoding='utf-8') as f:
        ...
        for s in samples:
            f.write(f'{s[0]}\t{s[1]}\t{df[df["报告内样本名称"]==s[0]]["标记"].values[0]}\t{s[2]}\n')
...
def create_annotations():
    ...
    with open(r'./table/annotations.txt', 'w', encoding='utf-8', newline='') as f:
        ...
        f.write(f'GO\t{GO}\t{round(GO / total * 100, 2)}%\n')
        f.write(f'KEGG\t{KEGG}\t{round(KEGG / total * 100, 2)}%\n')
        # f.write(f'Reactome\t{Reactome}\t{round(Reactome / total * 100, 2)}%\n')
        f.write(f'Domain\t{Interproscan}\t{round(Interproscan / total * 100, 2)}%\n')
        f.write(f'TF\t{TF}\t{round(TF / total * 100, 2)}%\n')
        f.write(f'Kinase\t{Kinase}\t{round(Kinase / total * 100, 2)}%\n')
        f.write(f'Subcellular Localization\t{SL}\t{round(SL / total * 100, 2)}%\n')
        f.write(f'eggNOG\t{eggNOG}\t{round(eggNOG / total * 100, 2)}%\n')
        f.write(f'DO\t{DO}\t{round(DO / total * 100, 2)}%\n')
        f.write(f'Wikipathway\t{Wikipathway}\t{round(Wikipathway / total * 100, 2)}%\n')
...
def create_md():
    with open('prot_report_TMT.md', 'w', encoding='utf-8', newline='') as md:
        with open(r'./temp/prot_report_test_TMT.md', 'r', encoding='utf-8') as f:
            for line in f:

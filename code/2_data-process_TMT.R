...
tag_relate <- read.xlsx("data/标记对照表.xlsx")
sample.name <- setNames(tag_relate$报告内样本名称,tag_relate$标记)
...
process_colnames <- function(data,selected_col,sample.name){
  ...
  names(newdata) <- names(newdata) %>% gsub("Abundances \\(Scaled\\): F1: ","",.)%>%gsub(", Sample","",.)
...
# 修改列名，过滤空值
protein.data.new <- protein.data %>%  filter(`# Unique Peptides` >= 1, `Score Sequest HT: Sequest HT` > 0) %>% 
...
na_rows <- rowSums(is.na(protein.data.new[, sample.name])) > 0
data_filtered <- protein.data.new[!na_rows, ]
protein_data_impute <- data_filtered %>% select(Protein.Accessions, Gene.Name, Protein.Description,`Molecular Weight[KDa]`,Isoelectric.Point,Coverage, everything())
pca.data <- protein_data_impute[,c("Protein.Accessions",sample.name)]
write.xlsx(pca.data, "temp/pca_data.xlsx")
nosample_col <- setdiff(names(protein_data_impute),sample.name)
## 按照分组计算 Mean

...
get_genelist <- function(diffExp_df, Anno_df, type) {
  ...
  # genelist_df <- genelist_df[!is.na(genelist_df$Genes),]

  # AnnotationDb <- AnnotationDbi::get(AnnotationDb)
  # genelist_df$entrezIDs <- as.character(AnnotationDbi::mget(as.vector(genelist_df$Genes), AnnotationDb, ifnotfound=NA)) 
  # genelist_df <- genelist_df[genelist_df$entrezIDs != "NA",]
  ...
  genelist <- genelist[names(genelist) %in% unique(Anno_df[,c("UniprotID")])]
...
GSEA_anno <- function(suboutdir, contrast, genelist, Anno_GO_df, group_info, minGSGOSize, maxGSGOSize, pvalueCutoff, pAdjustMethod, type) {
  ...
  GSEA_result <- GSEA(genelist,
                      TERM2GENE = as.data.frame(Anno_GO_df[, c("ReactomeID","UniprotID")]),
  ...
  match.index <- match(GSEA_df$ID, Anno_GO_df$ReactomeID)
  GSEA_df$Description <- Anno_GO_df$Reactome_term[match.index]
  ...
  plot_freqpoly <- ggplot(GSEA_df, aes(x = ES)) +
    ...
    labs(x = "Enrichment Score(ES)", y = "# of gene stes", title = paste0(group_info[1], "_versus_", group_info[2]))
...
main <- function(conf) {
  ...
  if (type == "Reactome") {
    ...
    # suppressWarnings(suppressMessages(library(org.Rn.eg.db)))
    # suppressWarnings(suppressMessages(library(reactome.db)))
    Reactome_db <- read.delim(Reactome_file_path,header = F)
    colnames(Reactome_db) <- c("UniprotID","ReactomeID","Reactome_url","Reactome_term","IEA","Species")
    organism_Scientific_name1 <- gsub(pattern = "_",replacement = " ",x = organism_Scientific_name)
    Anno_sub_df <- Reactome_db[Reactome_db$Species == organism_Scientific_name1,]
  }
  ## 对每个比较组的文件进行循环处理
  for (contrast in contrast_df[[1]]) {
    print(contrast)

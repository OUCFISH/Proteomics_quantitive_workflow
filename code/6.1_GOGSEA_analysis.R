...
GSEA_anno <- function(suboutdir, contrast, genelist, Anno_GO_df, group_info, minGSGOSize, maxGSGOSize, pvalueCutoff, pAdjustMethod, type) {
  ...
  GSEA_result <- GSEA(genelist,
                      TERM2GENE = as.data.frame(Anno_GO_df[, c("pathway", "Protein")]),
...
main <- function(conf) {
  ...
  GO_RData <- Sys.glob(paste0(uniprot_file_path,organism_category,"/*",organism_code, "_", organism_ID, "_godb.tsv"))
  
  # Anno_df <- read.xlsx(Annotation)
  if (type == "GO" && file.exists(GO_RData)) {
    term2gene <- read.delim(GO_RData)
    save(term2gene, file = "./temp/term2gene.RData")

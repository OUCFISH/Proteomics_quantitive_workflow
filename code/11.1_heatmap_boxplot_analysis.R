...
addAnnotation <- function(hpList, markGeneDf, DEGDt, exp, group, mapdt, color.ls, pp){
  hp <- hpList$hp
  ...
  if (pp$anno_box) {
    panel_fun <- function(index, nm) {
      grid::pushViewport(grid::viewport(xscale = c(-0.1,1.1), yscale = c(0,1)))
      grid::grid.rect()

      tmpmat <- mat[index, unlist(column_order(drawedHp)), drop = FALSE]

      # choose method
      if (boxMethod == "mean") {
        mdia <- colMeans(tmpmat)
      }else if (boxMethod == "median") {
      ...
      # box color
      if (is.null(boxColorGrp)) {
        boxColor <- rep("grey90",ncol(tmpmat))
      }

      # boxplot grobs
      lapply(colnames(tmpmat), function(x){
        if (group) {
          boxfillColor <- boxColorGrp[x]
        }else{
          boxfillColor <- boxColorGrp[mapdt[match(x, SampleName), Default]]
        }
        ComplexHeatmap::grid.boxplot(scales::rescale(tmpmat[, x],
                                                     to = c(0,1),
                                                     from = c(rg[1] - 0.5,rg[2] + 0.5)),
                                     pos = pos[which(colnames(tmpmat) %in% x)],
                                     direction = "vertical",
                                     box_width = as.numeric(boxWidth),
                                     outline = FALSE,
      ...
      if (genestat) {
        ...
        text <- paste("Protein Size:",nrow(mat[index, ]),sep = ' ')
        grid.text(text,x = 0.5, y = 0.9,
                  gp = grid::gpar(fontfamily = pp$anno_box_genestat_fontfamily,
                                  fontsize = pp$anno_box_genestat_fontsize,
                                  col = pp$anno_box_genestat_color
                                  # fontface = "italic"
                                  ))
      }
    ...
    boxMethod <- pp$anno_box_mline_method
    ...
    boxColorGrp <- structure(color.ls[[pp$anno_group_color]][1:length(uniqGrp)],names = uniqGrp)
    # boxColor <- boxColorGrp[grp]

    rg = range(mat)

    boxWidth <- pp$anno_boxplot_width

    # gene stat
    genestat <- pp$anno_box_genestat

    boxHa <- ComplexHeatmap::anno_link(align_to = subgroup,
                                      which = "row",
                                      panel_fun = panel_fun,
                                      size = grid::unit(pp$anno_box_size, "cm"),
                                      gap = grid::unit(pp$anno_box_gap, "cm"),
                                      width = grid::unit(pp$anno_box_width, "cm"),
                                      side = pp$anno_box_side,
                                      link_gp = grid::gpar(fill = "grey90",col = NA))
    
    if (pp$anno_box_side == "right") {
      rightAnnoList$boxHa <- boxHa
      rightAnnoList$annotation_name_gp <- gpar(fontsize = pp$hpb_legend_title_fontsize,
                                               fontfamily =  pp$hpb_legend_title_fontfamily)
      rightAnnoList$annotation_name_side <- pp$anno_geneSlice_name_side
      rightAnnoList$annotation_name_rot <- pp$hpb_colnames_rot
    }else{
      leftAnnoList$boxHa <- boxHa
      leftAnnoList$annotation_name_gp <- gpar(fontsize = pp$hpb_legend_title_fontsize,
                                              fontfamily =  pp$hpb_legend_title_fontfamily)
      leftAnnoList$annotation_name_side <- pp$anno_geneSlice_name_side
      leftAnnoList$annotation_name_rot <- pp$hpb_colnames_rot
  ...
  if (pp$anno_regulation_anno) {
    regDf <- DEGDt[match(rownames(exp), id)][,`:=`(id = NULL,Name = NULL)] %>% setDF()
...
main <- function(conf) {
  if (conf != "") {
    # source("11.1_heatmap_boxplot.conf")
    source(conf)
  }
  # 创建outdir
  ...
  hp <- addAnnotation(hpList = hpSubgroup, markGeneDf = markGeneDf,
                      DEGDt = DEGDt, exp = expDf,  group = group, mapdt = mapdt, 
                      color.ls = color.ls, pp = plotParas)
  ...
  save_plot(p = hp, outdir = outdir, saveName = "heatmapBoxplot",
            width = hp_size[1] + 1,
